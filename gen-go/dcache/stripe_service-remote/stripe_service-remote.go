...
func main() {
  ...
  client := dcache.NewStripeServiceClient(thrift.NewTStandardClient(iprot, oprot))
  ...
  case "PutStripe":
    ...
    value0 := argvalue0
    fmt.Print(client.PutStripe(context.Background(), value0))
    fmt.Print("\n")
    break
  case "RemoveStripe":
    if flag.NArg() - 1 != 1 {
      fmt.Fprintln(os.Stderr, "RemoveStripe requires 1 args")
      flag.Usage()
    }
    argvalue0 := flag.Arg(1)
    value0 := argvalue0
    fmt.Print(client.RemoveStripe(context.Background(), value0))
    fmt.Print("\n")
    break
  case "":
    Usage()
